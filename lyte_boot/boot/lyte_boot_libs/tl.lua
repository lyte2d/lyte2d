...
; local assert = _tl_compat and _tl_compat.assert or assert; local debug = _tl_compat and _tl_compat.debug or debug; local io =
...
_tl_compat.string or string; local table = _tl_compat and _tl_compat.table or table; local _tl_table_unpack = unpack or
table.unpack
local VERSION = "0.15.2+dev"

local tl = { TypeCheckOptions = {}, Env = {}, Symbol = {}, Result = {}, Error = {}, TypeInfo = {}, TypeReport = {},
    TypeReportEnv = {}, }
...
local function fail(ps, i, msg)
    ...
    table.insert(ps.errs,
        { filename = ps.filename, y = ps.tokens[i].y, x = ps.tokens[i].x,
    ...
    if ps.tokens[i].tk == "end" then
        ...
        if node.kind ~= "function" and endy ~= node.y and endx ~= node.x then
            if not ps.end_alignment_hint then
                ps.end_alignment_hint = { filename = ps.filename, y = node.y, x = node.x,
    ...
    if ps.tokens[i].tk == "=" then
        ...
        if next_word == "record" then
            ...
            return failskip(ps, i + 1,
                "syntax error: this syntax is no longer valid; use '" .. scope .. " record " .. asgn.vars[1].tk .. "'",
                skip_record)
        elseif next_word == "enum" then
            local scope = node_name == "local_declaration" and "local" or "global"
            return failskip(ps, i + 1,
                "syntax error: this syntax is no longer valid; use '" .. scope .. " enum " .. asgn.vars[1].tk .. "'",
                skip_enum)
        elseif next_word == "functiontype" then
            local scope = node_name == "local_declaration" and "local" or "global"
            return failskip(ps, i + 1,
...
local function init_globals(lax)
    ...
    local TABLE_SORT_FUNCTION = a_gfunction(1,
        function(a) return { args = TUPLE({ a, a }), rets = TUPLE({ BOOLEAN }) } end)
    ...
    local standard_library = {
        ...
        ["collectgarbage"] = a_type({
            ...
            types = {
                a_type({ typename = "function",
                    args = TUPLE({
                        a_type({ typename = "enum",
                            enumset = { ["collect"] = true, ["count"] = true, ["stop"] = true, ["restart"] = true } }) }),
                    rets = TUPLE({ NUMBER }) }),
                a_type({ typename = "function",
                    args = TUPLE({
                        a_type({ typename = "enum",
                            enumset = { ["step"] = true, ["setpause"] = true, ["setstepmul"] = true } }), NUMBER }),
                    rets = TUPLE({ NUMBER }) }),
                a_type({ typename = "function",
                    args = TUPLE({ a_type({ typename = "enum", enumset = { ["isrunning"] = true } }) }),
                    rets = TUPLE({ BOOLEAN }) }),
                a_type({ typename = "function", args = TUPLE({ STRING, OPT(NUMBER) }),
                    rets = TUPLE({ a_type({ typename = "union", types = { BOOLEAN, NUMBER } }) }) }),
            },
        }),
        ...
        ["load"] = a_type({ typename = "function", args = TUPLE({ UNION({ STRING, LOAD_FUNCTION }), OPT(STRING),
            OPT(STRING), OPT(TABLE) }), rets = TUPLE({ FUNCTION, STRING }) }),
        ["loadfile"] = a_type({ typename = "function", args = TUPLE({ OPT(STRING), OPT(STRING), OPT(TABLE) }), rets =
        ...
        ["next"] = a_type({
            ...
            types = {
                ...
                a_gfunction(1,
                    function(a) return { args = TUPLE({ ARRAY(a), OPT(a) }), rets = TUPLE({ INTEGER, a }) } end),
            },
        }),
        ...
        ["xpcall"] = a_type({ typename = "function", args = VARARG({ FUNCTION, XPCALL_MSGH_FUNCTION, ANY }), rets =
        VARARG({ BOOLEAN, ANY }) }),
        ...
        ["FILE"] = a_type({
            ...
            def = a_type({
                ...
                is_userdata = true,
                fields = {
                    ["close"] = a_type({ typename = "function", args = TUPLE({ NOMINAL_FILE }), rets = TUPLE({ BOOLEAN,
                        STRING }) }),
                    ["flush"] = a_type({ typename = "function", args = TUPLE({ NOMINAL_FILE }), rets = TUPLE({}) }),
                    ["lines"] = a_type({ typename = "function", args = VARARG({ NOMINAL_FILE,
                        a_type({ typename = "union", types = { STRING, NUMBER } }) }), rets = TUPLE({
                        a_type({ typename = "function", args = TUPLE({}), rets = VARARG({ STRING }) }),
                    }), }),
                    ["read"] = a_type({ typename = "function", args = TUPLE({ NOMINAL_FILE, UNION({ STRING, NUMBER }) }), rets =
                    TUPLE({ STRING, STRING }) }),
                    ["seek"] = a_type({ typename = "function", args = TUPLE({ NOMINAL_FILE, OPT(STRING), OPT(NUMBER) }), rets =
                    TUPLE({ INTEGER, STRING }) }),
                    ["setvbuf"] = a_type({ typename = "function", args = TUPLE({ NOMINAL_FILE, STRING, OPT(NUMBER) }), rets =
                    TUPLE({}) }),
                    ["write"] = a_type({ typename = "function", args = VARARG({ NOMINAL_FILE, STRING }), rets = TUPLE({
                        NOMINAL_FILE, STRING }) }),
                    ...
                    fields = {
                        ...
                        ["__pairs"] = a_gfunction(2, function(k, v)
                            return {
                                args = TUPLE({ a }),
                                rets = TUPLE({
                                    a_type({ typename = "function", args = TUPLE({}), rets = TUPLE({ k, v }) }) }),
                            }
                        end),
        ...
        ["debug"] = a_type({
            ...
            fields = {
                ...
                ["gethook"] = a_type({ typename = "function", args = TUPLE({ OPT(THREAD) }), rets = TUPLE({
                    DEBUG_HOOK_FUNCTION, INTEGER }) }),
                ["getlocal"] = a_type({
                    typename = "poly",
                    types = {
                ...
                ["getmetatable"] = a_gfunction(1, function(a) return { args = TUPLE({ a }),
                ...
                ["sethook"] = a_type({
                    typename = "poly",
                    types = {
                        a_type({ typename = "function", args = TUPLE({ THREAD, DEBUG_HOOK_FUNCTION, STRING, NUMBER }),
                            rets = TUPLE({}) }),
                        a_type({ typename = "function", args = TUPLE({ DEBUG_HOOK_FUNCTION, STRING, NUMBER }),
                            rets = TUPLE({}) }),
                    },
                }),
                ["setlocal"] = a_type({
                    typename = "poly",
                    types = {
                        a_type({ typename = "function", args = TUPLE({ THREAD, NUMBER, NUMBER, ANY }),
                            rets = TUPLE({ STRING }) }),
                        a_type({ typename = "function", args = TUPLE({ NUMBER, NUMBER, ANY }), rets = TUPLE({ STRING }) }),
                    },
                }),
                ["setmetatable"] = a_gfunction(1, function(a) return { args = TUPLE({ a, METATABLE(a) }),
                        rets = TUPLE({ a }) } end),
                ["setupvalue"] = a_type({ typename = "function", args = TUPLE({ FUNCTION, NUMBER, ANY }), rets = TUPLE({
                    STRING }) }),
                ["setuservalue"] = a_type({ typename = "function", args = TUPLE({ USERDATA, ANY, NUMBER }), rets = TUPLE({
                    USERDATA }) }),
                ["traceback"] = a_type({
                    typename = "poly",
                    types = {
                        a_type({ typename = "function", args = TUPLE({ THREAD, STRING, NUMBER }),
                            rets = TUPLE({ STRING }) }),
                        a_type({ typename = "function", args = TUPLE({ STRING, NUMBER }), rets = TUPLE({ STRING }) }),
                    },
                }),
                ["upvalueid"] = a_type({ typename = "function", args = TUPLE({ FUNCTION, NUMBER }), rets = TUPLE({
                    USERDATA }) }),
                ["upvaluejoin"] = a_type({ typename = "function", args = TUPLE({ FUNCTION, NUMBER, FUNCTION, NUMBER }), rets =
                TUPLE({}) }),
                ["getinfo"] = a_type({
                    typename = "poly",
                    types = {
                        a_type({ typename = "function", args = TUPLE({ ANY }), rets = TUPLE({ DEBUG_GETINFO_TABLE }) }),
                        a_type({ typename = "function", args = TUPLE({ ANY, STRING }),
                            rets = TUPLE({ DEBUG_GETINFO_TABLE }) }),
                        a_type({ typename = "function", args = TUPLE({ ANY, ANY, STRING }),
                            rets = TUPLE({ DEBUG_GETINFO_TABLE }) }),
                    },
                }),
            },
        ...
        ["io"] = a_type({
            typename = "record",
            fields = {
                ["close"] = a_type({ typename = "function", args = TUPLE({ OPT(NOMINAL_FILE) }), rets = TUPLE({ BOOLEAN,
                    STRING }) }),
                ["flush"] = a_type({ typename = "function", args = TUPLE({}), rets = TUPLE({}) }),
                ["input"] = a_type({ typename = "function", args = TUPLE({ OPT(UNION({ STRING, NOMINAL_FILE })) }), rets =
                TUPLE({ NOMINAL_FILE }) }),
                ["lines"] = a_type({ typename = "function", args = VARARG({ OPT(STRING),
                    a_type({ typename = "union", types = { STRING, NUMBER } }) }), rets = TUPLE({
                    a_type({ typename = "function", args = TUPLE({}), rets = VARARG({ STRING }) }),
                }), }),
                ["open"] = a_type({ typename = "function", args = TUPLE({ STRING, STRING }), rets = TUPLE({ NOMINAL_FILE,
                    STRING }) }),
                ["output"] = a_type({ typename = "function", args = TUPLE({ OPT(UNION({ STRING, NOMINAL_FILE })) }), rets =
                TUPLE({ NOMINAL_FILE }) }),
                ["popen"] = a_type({ typename = "function", args = TUPLE({ STRING, STRING }), rets = TUPLE({
                    NOMINAL_FILE, STRING }) }),
                ["read"] = a_type({ typename = "function", args = TUPLE({ UNION({ STRING, NUMBER }) }), rets = TUPLE({
                    STRING, STRING }) }),
                ["stderr"] = NOMINAL_FILE,
                ["stdin"] = NOMINAL_FILE,
                ["stdout"] = NOMINAL_FILE,
                ...
                ["type"] = a_type({ typename = "function", args = TUPLE({ ANY }), rets = TUPLE({ STRING }) }),
                ["write"] = a_type({ typename = "function", args = VARARG({ STRING }), rets = TUPLE({ NOMINAL_FILE,
                    STRING }) }),
            },
        }),
        ["math"] = a_type({
            ...
            fields = {
                ...
                ["max"] = a_type({
                    ...
                    types = {
                        ...
                        a_type({ typename = "function",
                            args = VARARG({ a_type({ typename = "union", types = { NUMBER, INTEGER } }) }),
                            rets = TUPLE({ NUMBER }) }),
                ...
                ["randomseed"] = a_type({ typename = "function", args = TUPLE({ NUMBER, NUMBER }), rets = TUPLE({
                    INTEGER, INTEGER }) }),
            ...
            fields = {
                ...
                ["date"] = a_type({
                    ...
                    types = {
                        ...
                        a_type({ typename = "function", args = TUPLE({ OS_DATE_TABLE_FORMAT, NUMBER }),
                            rets = TUPLE({ OS_DATE_TABLE }) }),
                        a_type({ typename = "function", args = TUPLE({ OPT(STRING), OPT(NUMBER) }),
                            rets = TUPLE({ STRING }) }),
                    },
                }),
                ["difftime"] = a_type({ typename = "function", args = TUPLE({ NUMBER, NUMBER }), rets = TUPLE({ NUMBER }) }),
                ["execute"] = a_type({ typename = "function", args = TUPLE({ STRING }), rets = TUPLE({ BOOLEAN, STRING,
                    INTEGER }) }),
                ["exit"] = a_type({ typename = "function", args = TUPLE({ UNION({ NUMBER, BOOLEAN }), BOOLEAN }), rets =
                TUPLE({}) }),
                ...
                ["rename"] = a_type({ typename = "function", args = TUPLE({ STRING, STRING }), rets = TUPLE({ BOOLEAN,
                    STRING }) }),
                ["setlocale"] = a_type({ typename = "function", args = TUPLE({ STRING, OPT(STRING) }), rets = TUPLE({
                    STRING }) }),
        ...
        ["package"] = a_type({
            ...
            fields = {
                ...
                ["searchpath"] = a_type({ typename = "function", args = TUPLE({ STRING, STRING, OPT(STRING), OPT(STRING) }), rets =
                TUPLE({ STRING, STRING }) }),
            },
        }),
        ["string"] = a_type({
            ...
            fields = {
                ["byte"] = a_type({
                    ...
                    types = {
                        ...
                        a_type({ typename = "function", args = TUPLE({ STRING, NUMBER, NUMBER }),
                            rets = VARARG({ INTEGER }) }),
                    },
                }),
                ["char"] = a_type({ typename = "function", args = VARARG({ NUMBER }), rets = TUPLE({ STRING }) }),
                ["dump"] = a_type({ typename = "function", args = TUPLE({ FUNCTION, OPT(BOOLEAN) }), rets = TUPLE({
                    STRING }) }),
                ["find"] = a_type({ typename = "function", args = TUPLE({ STRING, STRING, OPT(NUMBER), OPT(BOOLEAN) }), rets =
                VARARG({ INTEGER, INTEGER, STRING }) }),
                ...
                ["gsub"] = a_type({
                    typename = "poly",
                    types = {
                        a_type({ typename = "function", args = TUPLE({ STRING, STRING, STRING, NUMBER }),
                            rets = TUPLE({ STRING, INTEGER }) }),
                        a_type({ typename = "function",
                            args = TUPLE({ STRING, STRING, a_type({ typename = "map", keys = STRING, values = STRING }),
                                NUMBER }), rets = TUPLE({ STRING, INTEGER }) }),
                        a_type({ typename = "function",
                            args = TUPLE({ STRING, STRING,
                                a_type({ typename = "function", args = VARARG({ STRING }), rets = TUPLE({ STRING }) }) }),
                            rets = TUPLE({ STRING, INTEGER }) }),
                        a_type({ typename = "function",
                            args = TUPLE({ STRING, STRING,
                                a_type({ typename = "function", args = VARARG({ STRING }), rets = TUPLE({ NUMBER }) }) }),
                            rets = TUPLE({ STRING, INTEGER }) }),
                        a_type({ typename = "function",
                            args = TUPLE({ STRING, STRING,
                                a_type({ typename = "function", args = VARARG({ STRING }), rets = TUPLE({ BOOLEAN }) }) }),
                            rets = TUPLE({ STRING, INTEGER }) }),
                        a_type({ typename = "function",
                            args = TUPLE({ STRING, STRING,
                                a_type({ typename = "function", args = VARARG({ STRING }), rets = TUPLE({}) }) }),
                            rets = TUPLE({ STRING, INTEGER }) }),
                ...
                ["match"] = a_type({ typename = "function", args = TUPLE({ STRING, STRING, NUMBER }), rets = VARARG({
                    STRING }) }),
                ...
                ["rep"] = a_type({ typename = "function", args = TUPLE({ STRING, NUMBER, OPT(STRING) }), rets = TUPLE({
                    STRING }) }),
                ...
                ["unpack"] = a_type({ typename = "function", args = TUPLE({ STRING, STRING, OPT(NUMBER) }), rets = VARARG({
                    ANY }) }),
        ...
        ["table"] = a_type({
            typename = "record",
            fields = {
                ["concat"] = a_type({ typename = "function", args = TUPLE({ ARRAY(UNION({ STRING, NUMBER })),
                    OPT(STRING), OPT(NUMBER), OPT(NUMBER) }), rets = TUPLE({ STRING }) }),
                ["insert"] = a_type({
                    typename = "poly",
                    types = {
                        a_gfunction(1, function(a) return { args = TUPLE({ ARRAY(a), NUMBER, a }), rets = TUPLE({}) } end),
                ...
                ["move"] = a_type({
                    ...
                    types = {
                        a_gfunction(1,
                            function(a) return { args = TUPLE({ ARRAY(a), NUMBER, NUMBER, NUMBER }),
                                    rets = TUPLE({ ARRAY(a) }) } end),
                        a_gfunction(1,
                            function(a) return { args = TUPLE({ ARRAY(a), NUMBER, NUMBER, NUMBER, ARRAY(a) }),
                                    rets = TUPLE({ ARRAY(a) }) } end),
                    },
                }),
                ["pack"] = a_type({ typename = "function", args = VARARG({ ANY }), rets = TUPLE({ TABLE }) }),
                ["remove"] = a_gfunction(1, function(a) return { args = TUPLE({ ARRAY(a), OPT(NUMBER) }),
                        rets = TUPLE({ a }) } end),
                ["sort"] = a_gfunction(1, function(a) return { args = TUPLE({ ARRAY(a), OPT(TABLE_SORT_FUNCTION) }),
                        rets = TUPLE({}) } end),
                ["unpack"] = a_gfunction(1, function(a) return { needs_compat = true,
                        args = TUPLE({ ARRAY(a), NUMBER, NUMBER }), rets = VARARG({ a }) } end),
            },
        }),
        ["utf8"] = a_type({
            typename = "record",
            fields = {
                ["char"] = a_type({ typename = "function", args = VARARG({ NUMBER }), rets = TUPLE({ STRING }) }),
                ["charpattern"] = STRING,
                ["codepoint"] = a_type({ typename = "function", args = TUPLE({ STRING, OPT(NUMBER), OPT(NUMBER) }), rets =
                VARARG({ INTEGER }) }),
                ["codes"] = a_type({ typename = "function", args = TUPLE({ STRING }), rets = TUPLE({
                    a_type({ typename = "function", args = TUPLE({}), rets = TUPLE({ NUMBER, STRING }) }),
                }), }),
                ["len"] = a_type({ typename = "function", args = TUPLE({ STRING, NUMBER, NUMBER }), rets = TUPLE({
                    INTEGER }) }),
                ["offset"] = a_type({ typename = "function", args = TUPLE({ STRING, NUMBER, NUMBER }), rets = TUPLE({
                    INTEGER }) }),
            },
        }),
        ["_VERSION"] = STRING,
...
tl.type_check = function(ast, opts)
    ...
    local function redeclaration_warning(node, old_var)
        ...
        if old_var and old_var.declared_at then
            node_warning("redeclaration", node, short_error .. " (originally declared at %d:%d)", var_name,
        ...
        elseif t1.typename == "function" then
            ...
            if #t1.args ~= #t2.args then
                if t1.is_method ~= t2.is_method then
                    return false,
                        terr(t1, "different number of input arguments: method and non-method are not the same type")
                end
                return false,
                    terr(t1,
                        "different number of input arguments: got " ..
                        #t1.args - argdelta .. ", expected " .. #t2.args - argdelta)
            end
            if #t1.rets ~= #t2.rets then
                return false,
                    terr(t1, "different number of return values: got " .. #t1.rets .. ", expected " .. #t2.rets)
            end
            local all_errs = {}
            for i = 1, #t1.args do
                ...
                if t2.inferred_len and t2.inferred_len > #t1.types then
                    return false,
                        terr(t1,
                            ...
                            tostring(#t1.types) .. ", got " .. tostring(t2.inferred_len))
                end
            ...
            if t1.typename == "tupletable" then
                for i = 1, math.min(#t1.types, #t2.types) do
                    if not is_a(t1.types[i], t2.types[i], for_equality) then
                        return false,
                            terr(t1, "in tuple entry " .. tostring(i) .. ": got %s, expected %s", t1.types[i],
                                t2.types[i])
                    end
                end
            ...
            elseif is_array_type(t1) then
                if t1.inferred_len and t1.inferred_len > #t2.types then
                    return false,
                        terr(t1,
                            "incompatible length, expected maximum length of " ..
                            tostring(#t2.types) .. ", got " .. tostring(t1.inferred_len))
                end
                ...
                for i = 1, len do
                    if not is_a(t1.elements, t2.types[i], for_equality) then
                        return false,
                            terr(t1,
                                "tuple entry " ..
                                tostring(i) .. " of type %s does not match type of array elements, which is %s",
        ...
        elseif t1.typename == "function" and t2.typename == "function" then
            ...
            if (not t2.args.is_va) and #t1.args > #t2.args then
                table.insert(all_errs,
                    error_in_type(t1,
                        "incompatible number of arguments: got " .. #t1.args .. " %s, expected " .. #t2.args .. " %s",
                        t1.args, t2.args))
            else
    ...
    local function assert_is_a(node, t1, t2, context, name)
        ...
        elseif t2.typename == "unresolved_emptytable_value" then
            if is_number_type(t2.emptytable_type.keys) then
                ...
                infer_emptytable(t2.emptytable_type,
                    infer_at(node, a_type({ typename = "map", keys = t2.emptytable_type.keys, values = t1 })))
            end
            return true
        elseif t2.typename == "emptytable" then
            ...
            elseif t1.typename ~= "emptytable" then
                node_error(node,
                    context .. ": " .. (name and (name .. ": ") or "") .. "assigning %s to a variable declared with {}",
                    t1)
                return false
            end
            return true
    ...
    do
            ...
            if f.typeargs then
                for _, a in ipairs(f.typeargs) do
                    if not find_var_type(a.typearg) then
                        add_var(nil, a.typearg,
                            lax and UNKNOWN or { typename = "unresolvable_typearg", typearg = a.typearg })
                    end
                end
            end
        end
        ...
        local function fail_call(node, func, nargs, errs)
            if errs then
                ...
                node_error(node,
                    "wrong number of arguments (given " .. nargs .. ", expects " .. table.concat(expects, " or ") .. ")")
            end
                    ...
                    if (not tried) or not tried[i] then
                        ...
                        if f.is_method and not is_method then
                            if args[1] and is_a(args[1], f.args[1]) then
                                local receiver_is_typetype = where.e1.e1 and where.e1.e1.type and
                                where.e1.e1.type.resolved and where.e1.e1.type.resolved.typename == "typetype"
                                if not receiver_is_typetype then
                                    node_warning("hint", where,
    ...
    local function add_global(node, var, valtype, is_assigning)
        ...
        if existing then
            ...
            if valtype and not same_type(existing.t, valtype) then
                node_error(node, "cannot redeclare global with a different type: previous type of " .. var .. " is %s",
                    existing.t)
            end
            return nil
        end
    ...
    local function check_metamethod(node, op, a, b)
        ...
        if metamethod then
            return resolve_tuple_and_nominal(type_check_function_call(node, where_args, metamethod, args, nil, false, 0)),
                meta_on_operator
        ...
        if key ~= nil then
            ...
            if s then
                node_error(node,
                    in_context(ctx,
                        "redeclared key " ..
                        tostring(key) .. " (previously declared at " .. filename .. ":" .. s.y .. ":" .. s.x .. ")"))
            else
                seen_keys[key] = node
            end
        end
        ...
        ["return"] = {
            ...
            after = function(node, children)
                ...
                if #children[1] > nrets and (not lax) and not vatype then
                    node_error(node,
                        "in " ..
                        what .. ": excess return values, expected " .. #rets .. " %s, got " .. #children[1] .. " %s",
                        rets, children[1])
                end
                ...
                if nrets > 1 and
                    ...
                    node_warning("hint", node.exps[1].e2,
                ...
                if node.expected then
                    ...
                    for i, child in ipairs(children) do
                        ...
                        elseif is_tupletable and is_number_type(child.ktype) then
                            ...
                            elseif not dt then
                                node_error(node[i],
                                    in_context(node.expected_context, "unexpected index " .. n .. " in tuple %s"),
                                    decltype)
                            else
                        ...
                        elseif is_map then
                            ...
                            node_error(node[i],
                                in_context(node.expected_context, "unexpected key of type %s in table of type %s"),
                ...
                if rfieldtype then
                    ...
                    if not ok then
                        if rfieldtype.typename == "poly" then
                            add_errs_prefixing(node, err, errors,
                ...
                elseif node.op.arity == 2 and binop_types[node.op.op] then
                    ...
                    if not node.type then
                        ...
                        if not node.type then
                            return node_error(node,
                                ...
                                resolve_tuple(orig_a), resolve_tuple(orig_b))
                        end
                    end
                    ...
                    if node.op.op == "//" and env.gen_target == "5.1" then
                        if meta_on_operator then
                            ...
                            local div = { y = node.y, x = node.x, kind = "op", op = an_operator(node, 2, "/"),
                                e1 = node.e1, e2 = node.e2 }
                            convert_node_to_compat_call(node, "math", "floor", div)
                        end
                    elseif bit_operators[node.op.op] and env.gen_target == "5.1" then
                        if meta_on_operator then
                            all_needs_compat["mt"] = true
                            convert_node_to_compat_mt_call(node, binop_to_metamethod[node.op.op], meta_on_operator,
                                node.e1, node.e2)
                        else
                            all_needs_compat["bit32"] = true
